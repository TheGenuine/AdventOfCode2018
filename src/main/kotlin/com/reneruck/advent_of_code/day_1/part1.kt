...
fun parseInput(input : List<String>) : List<Int> = input.map { Integer.parseInt(it) }

fun calculateFrequency() : Int =
        loadInput("day_1/input.txt")
                .attempt()
                .map { it.fold({listOf<Int>()},{ parseInput(it)}) }
                .map { it.fold(0) { sum, freqMod ->  sum + freqMod } }
                .unsafeRunSync()

object Part1 {
        @JvmStatic
        fun main(args: Array<String>) {
                println(calculateFrequency())
        }
}